...
preserve
collapse worldregion6 countryGDPclass, by(country) 	
restore
...
svy, subpop(analytic_pop_diab): proportion treat_diagdia		
...
*** FIGURE 2: MEDICATION USE PATTERNS AMONG ALL DIABETICS BY COUNTRY ***				// Note: Because we do not report CIs in graph 2, including only pweights (and not psu+stratum & no subpop option) generates the same estimates, because psu & stratum in svyset as well as svy,subpop only affect the calculation (clustering) of standard errors
...
proportion treat_alldia_new [pw = weq_w3_analytic_all] 	
...
svy, subpop(analytic_pop_diab) over(countryGDPclass): proportion diag	
...
svy, subpop(analytic_pop_diab_diag) over(countryGDPclass): proportion treat_diagdia 	// verify numbers in graph --> check
