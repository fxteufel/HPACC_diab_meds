...
// See do-file HPACC_dia_meds_subset
...
forval x = 1/5 { 
svy, subpop(if analytic_pop_diab == 1 & age_cat2 == `x'): proportion diag
matrix est = e(b) 
gen B`x' = est[1,2]
global C`x' = 0.5*360*B`x'
}

graph pie diag undiag [pw = weq_w3_analytic_all] if analytic_pop_diab == 1 & age_cat2 == 1, graphregion(color(white)) ysize(4) xsize(4) plabel(_all percent, size(small) format(%9.1f) gap(35)) ///
...
graph pie diag undiag [pw = weq_w3_analytic_all] if analytic_pop_diab == 1 & age_cat2 == 2, graphregion(color(white)) ysize(4) xsize(4) plabel(_all percent, size(small) format(%9.1f) gap(35)) ///
...
graph pie diag undiag [pw = weq_w3_analytic_all] if analytic_pop_diab == 1 & age_cat2 == 3, graphregion(color(white)) ysize(4) xsize(4) plabel(_all percent, size(small) format(%9.1f) gap(35)) ///
...
graph pie diag undiag [pw = weq_w3_analytic_all] if analytic_pop_diab == 1 & age_cat2 == 4, graphregion(color(white)) ysize(4) xsize(4) plabel(_all percent, size(small) format(%9.1f) gap(35)) ///
pie(1, explode(2) color(lavender)) pie(2, color(edkblue)) fysize(40) fxsize(40) angle0($C4) legend(off) title(" ", size(small) position(11)) l1title("{bf}55-64", size(small) col(black) ring(0) /*orientation(horizontal)*/) ///
b1title(" ", size(small))
graph save "pie4_h", replace

graph pie diag undiag [pw = weq_w3_analytic_all] if analytic_pop_diab == 1 & age_cat2 == 5, graphregion(color(white)) ysize(4) xsize(4) plabel(_all percent, size(small) format(%9.1f) gap(35)) ///
legend(position(7) ring(0) order(2 "Undiagnosed" 1 "Diagnosed") symy(2) symx(2) textw(8) size(vsmall) col(1) keygap(*0.3)) ///
pie(1, explode(2) color(lavender)) pie(2, color(edkblue)) fysize(40) fxsize(40) angle0($C5) ///
l1title("{bf}>64", size(small) col(black) ring(0) /*orientation(horizontal)*/) ///
b1title(" ", size(small)) title(" ", size(small))
graph save "pie5_h", replace

svy, subpop(analytic_pop_diab) over(age_cat2): proportion diag		// verify numbers in graph --> check
...
svy, subpop(if analytic_pop_diab_diag == 1 & age_cat2 == `x'): proportion nomeds_diagdia 
matrix est = e(b)	
matrix ci = r(table)
replace proportions_diag`x' = est[1, 2] if treat_diagdia_inv == 3
replace lower_bound_diag`x' = ci[5,2] if treat_diagdia_inv == 3
...
svy, subpop(if analytic_pop_diab_diag == 1 & age_cat2 == `x'): proportion oral_diagdia 
matrix est = e(b)
matrix ci = r(table)
replace proportions_diag`x' = est[1, 2] if treat_diagdia_inv == 2
replace lower_bound_diag`x' = ci[5,2] if treat_diagdia_inv == 2
...
svy, subpop(if analytic_pop_diab_diag == 1 & age_cat2 == `x'): proportion orl_insl_diagdia 
matrix est = e(b)
matrix ci = r(table)
replace proportions_diag`x' = est[1, 2] if treat_diagdia_inv == 1
replace lower_bound_diag`x' = ci[5,2] if treat_diagdia_inv == 1
...
svy, subpop(if analytic_pop_diab_diag == 1 & age_cat2 == `x'): proportion insul_diagdia 
matrix est = e(b)
matrix ci = r(table)
replace proportions_diag`x' = est[1, 2] if treat_diagdia_inv == 0
replace lower_bound_diag`x' = ci[5,2] if treat_diagdia_inv == 0
...
twoway (bar proportions_diag1 treat_diagdia_inv if nomeds_diagdia == 1 & age_cat2 == 1, bcolor(eltblue) barw(0.95) horizontal)  ///
(bar proportions_diag1 treat_diagdia_inv if oral_diagdia == 1 & age_cat2 == 1, bcolor(orange) barw(0.95) horizontal)  ///
(bar proportions_diag1 treat_diagdia_inv if orl_insl_diagdia == 1 & age_cat2 == 1, bcolor(eltgreen) barw(0.95) horizontal)  ///
(bar proportions_diag1 treat_diagdia_inv if insul_diagdia == 1 & age_cat2 == 1, bcolor(gold) barw(0.95) horizontal) ///
(rcap upper_bound_diag1 lower_bound_diag1 treat_diagdia_inv if age_cat2 == 1, lcolor(gs8) horizontal) ///
(scatter treat_diagdia_inv proportions_diag1 if age_cat2 == 1, msymbol(none) mlabel(proportions_marker_diag1) mlabposition(1) mlabcolor(black) mlabsize(small)), ///			
ytitle("") ylabel(none) xtitle(" ", size(small)) xscale(range(0 100) titlegap(*1.2)) xlabel(0(10)100,labsize(vsmall) format(%9.0f) nogrid angle(0)) ///		
legend(off) ///
title("{bf}B) Individuals with diagnosed diabetes", size(small) col(black) position(11)) graphregion(color(white)) subtitle(" ", size(minuscule)) 
graph save "bar1_h", replace
...
forval x = 2/4 {
twoway (bar proportions_diag`x' treat_diagdia_inv if nomeds_diagdia == 1 & age_cat2 == `x', bcolor(eltblue) barw(0.95) horizontal)  ///
(bar proportions_diag`x' treat_diagdia_inv if oral_diagdia == 1 & age_cat2 == `x', bcolor(orange) barw(0.95) horizontal)  ///
(bar proportions_diag`x' treat_diagdia_inv if orl_insl_diagdia == 1 & age_cat2 == `x', bcolor(eltgreen) barw(0.95) horizontal)  ///
(bar proportions_diag`x' treat_diagdia_inv if insul_diagdia == 1 & age_cat2 == `x', bcolor(gold) barw(0.95) horizontal) ///
(rcap upper_bound_diag`x' lower_bound_diag`x' treat_diagdia_inv if age_cat2 == `x', lcolor(gs8) horizontal) ///
(scatter treat_diagdia_inv proportions_diag`x' if age_cat2 == `x', msymbol(none) mlabel(proportions_marker_diag`x') mlabposition(1) mlabcolor(black) mlabsize(small)), ///			
ytitle("") ylabel(none) xtitle(" ", size(small)) xscale(range(0 100) titlegap(*1.2)) xlabel(0(10)100,labsize(vsmall) format(%9.0f) nogrid angle(0)) ///		
title(" ", size(medsmall) position(11)) graphregion(color(white)) subtitle(" ", size(minuscule)) legend(off) 
graph save bar`x'_h, replace
}

twoway (bar proportions_diag5 treat_diagdia_inv if nomeds_diagdia == 1 & age_cat2 == 5, bcolor(eltblue) barw(0.95) horizontal)  ///
(bar proportions_diag5 treat_diagdia_inv if oral_diagdia == 1 & age_cat2 == 5, bcolor(orange) barw(0.95) horizontal)  ///
(bar proportions_diag5 treat_diagdia_inv if orl_insl_diagdia == 1 & age_cat2 == 5, bcolor(eltgreen) barw(0.95) horizontal)  ///
(bar proportions_diag5 treat_diagdia_inv if insul_diagdia == 1 & age_cat2 == 5, bcolor(gold) barw(0.95) horizontal) ///
(rcap upper_bound_diag5 lower_bound_diag5 treat_diagdia_inv if age_cat2 == 5, lcolor(gs8) horizontal) ///
(scatter treat_diagdia_inv proportions_diag5 if age_cat2 == 5, msymbol(none) mlabel(proportions_marker_diag5) mlabposition(1) mlabcolor(black) mlabsize(small)), ///			
ytitle("") ylabel(none) xtitle("Weighted proportion (%)", size(vsmall)) xscale(range(0 100) titlegap(*1.2)) xlabel(0(10)100,labsize(vsmall) format(%9.0f) nogrid angle(0)) ///		
title(" ", size(medsmall) position(11)) graphregion(color(white)) subtitle(" ", size(minuscule)) ///
...
svy, subpop(analytic_pop_diab) over(age_cat2): proportion treat_diagdia	// verify numbers in graph --> check


*** FIGURE S6: STRATIFICATION BY SURVEY YEAR ***
...
* All
forval x = 0/1 { 
svy, subpop(if analytic_pop_diab == 1 & year_grp == `x'): proportion diag
matrix est = e(b) 
gen B`x' = est[1,2]
global C`x' = 0.5*360*B`x'
}

graph pie diag undiag [pw = weq_w3_analytic_all] if analytic_pop_diab == 1 & year_grp == 0, graphregion(color(white)) ysize(4) xsize(4) plabel(_all percent, size(small) format(%9.1f)) ///
pie(1, explode(2) color(lavender)) pie(2, color(edkblue)) fysize(50) fxsize(50) angle0($C0) legend(off) title(" ", size(medsmall) position(11)) l1title("{bf}2009 to 2014", size(medsmall) col(black) ring(0) /*orientation(horizontal)*/) ///
b1title(" ", size(large)) title("{bf}A) All individuals with diabetes", size(medsmall) col(black) position(11)) 
graph save "pie1_h", replace

graph pie diag undiag [pw = weq_w3_analytic_all] if analytic_pop_diab == 1 & year_grp == 1, graphregion(color(white)) ysize(4) xsize(4) plabel(_all percent, size(small) format(%9.1f)) ///
legend(position(7) ring(0) order(2 "Undiagnosed" 1 "Diagnosed") symy(2) symx(2) textw(11) size(small) col(1) keygap(*0.5)) ///
pie(1, explode(2) color(lavender)) pie(2, color(edkblue)) fysize(50) fxsize(50) angle0($C1) ///
l1title("{bf}2015 to 2019", size(medsmall) col(black) ring(0) /*orientation(horizontal)*/) ///
b1title(" ", size(large)) title(" ", size(medsmall))
graph save "pie2_h", replace

svy, subpop(analytic_pop_diab) over(year_grp): proportion diag	// verify numbers in graph --> check
...
svy, subpop(if analytic_pop_diab_diag == 1 & year_grp == `x'): proportion nomeds_diagdia 
matrix est = e(b)	
matrix ci = r(table)
replace proportions_diag`x' = est[1, 2] if treat_diagdia_inv == 3
replace lower_bound_diag`x' = ci[5,2] if treat_diagdia_inv == 3
...
svy, subpop(if analytic_pop_diab_diag == 1 & year_grp == `x'): proportion oral_diagdia
matrix est = e(b)
matrix ci = r(table)
replace proportions_diag`x' = est[1, 2] if treat_diagdia_inv == 2
replace lower_bound_diag`x' = ci[5,2] if treat_diagdia_inv == 2
...
svy, subpop(if analytic_pop_diab_diag == 1 & year_grp == `x'): proportion orl_insl_diagdia 
matrix est = e(b)
matrix ci = r(table)
replace proportions_diag`x' = est[1, 2] if treat_diagdia_inv == 1
replace lower_bound_diag`x' = ci[5,2] if treat_diagdia_inv == 1
...
svy, subpop(if analytic_pop_diab_diag == 1 & year_grp == `x'): proportion insul_diagdia
matrix est = e(b)
matrix ci = r(table)
replace proportions_diag`x' = est[1, 2] if treat_diagdia_inv == 0
replace lower_bound_diag`x' = ci[5,2] if treat_diagdia_inv == 0
...
twoway (bar proportions_diag0 treat_diagdia_inv if nomeds_diagdia == 1 & year_grp == 0, bcolor(eltblue) barw(0.95) horizontal)  ///
(bar proportions_diag0 treat_diagdia_inv if oral_diagdia == 1 & year_grp == 0, bcolor(orange) barw(0.95) horizontal)  ///
(bar proportions_diag0 treat_diagdia_inv if orl_insl_diagdia == 1 & year_grp == 0, bcolor(eltgreen) barw(0.95) horizontal)  ///
(bar proportions_diag0 treat_diagdia_inv if insul_diagdia == 1 & year_grp == 0, bcolor(gold) barw(0.95) horizontal) ///
(rcap upper_bound_diag0 lower_bound_diag0 treat_diagdia_inv if year_grp == 0, lcolor(gs8) horizontal) ///
(scatter treat_diagdia_inv proportions_diag0 if year_grp == 0, msymbol(none) mlabel(proportions_marker_diag0) mlabposition(1) mlabcolor(black) mlabsize(small)), ///			
ytitle("") ylabel(none) xtitle(" ", size(small)) xscale(range(0 100) titlegap(*1.2)) xlabel(0(10)100,labsize(small) format(%9.0f) nogrid angle(0)) ///		
legend(off) ///
title("{bf}B) Individuals with diagnosed diabetes", size(medsmall) col(black) position(11)) graphregion(color(white)) subtitle(" ", size(minuscule)) 
graph save "bar1_h", replace

twoway (bar proportions_diag1 treat_diagdia_inv if nomeds_diagdia == 1 & year_grp == 1, bcolor(eltblue) barw(0.95) horizontal)  ///
(bar proportions_diag1 treat_diagdia_inv if oral_diagdia == 1 & year_grp == 1, bcolor(orange) barw(0.95) horizontal)  ///
(bar proportions_diag1 treat_diagdia_inv if orl_insl_diagdia == 1 & year_grp == 1, bcolor(eltgreen) barw(0.95) horizontal)  ///
(bar proportions_diag1 treat_diagdia_inv if insul_diagdia == 1 & year_grp == 1, bcolor(gold) barw(0.95) horizontal) ///
(rcap upper_bound_diag1 lower_bound_diag1 treat_diagdia_inv if year_grp == 1, lcolor(gs8) horizontal) ///
(scatter treat_diagdia_inv proportions_diag1 if year_grp == 1, msymbol(none) mlabel(proportions_marker_diag1) mlabposition(1) mlabcolor(black) mlabsize(small)), ///			
ytitle("") ylabel(none) xtitle("Weighted proportion (%)", size(small)) xscale(range(0 100) titlegap(*1.2)) xlabel(0(10)100,labsize(small) format(%9.0f) nogrid angle(0)) ///		
title(" ", size(medsmall) position(11)) graphregion(color(white)) subtitle(" ", size(minuscule)) ///
legend(position(5) ring(0) order(1 "No medication" 2 "Oral only" 3 "Oral and insulin" 4 "Insulin only") row(2) symy(2) symx(2) textw(11) size(small) rowgap(*0.5) colgap(*0.7) keygap(*0.5)) 
graph save "bar2_h", replace

svy, subpop(analytic_pop_diab_diag) over(year_grp): proportion treat_diagdia 	

* Combine
graph combine "pie1_h" "bar1_h" "pie2_h" "bar2_h", row(2) graphregion(margin(small)color("white")) imargin(0 0 0 0)
graph save "Year", replace 


*** FIGURE S7: WPOP INSTEAD OF WEQ ***
...
svy, subpop(analytic_pop_diab): proportion diag		
matrix est = e(b) 
gen B = .
replace B = est[1,2]
global C = 0.5*360*B
...
putexcel set par_charact.xls, sheet(Pooled) modify 

** Sex, Year **												
local i=-17
foreach var in sex year_grp {
local i=`i'+17
foreach x of num 0/1 {
* All
tab `var' if `var' == `x' & analytic_pop_all == 1
putexcel C`=6+`i'+`x'' = `r(N)'
svy, subpop(analytic_pop_all): proportion `var'
matrix results = e(b)
matrix m`x' = results[1...,`=1+`x''..`=1+`x'']
putexcel D`=6+`i'+`x'' = matrix(m`x'), nformat("0.0%")
* Diabetes
tab `var' if `var' == `x' & analytic_pop_diab == 1
putexcel E`=6+`i'+`x'' = `r(N)'
svy, subpop(analytic_pop_diab): proportion `var' 
matrix results = e(b)
matrix m`x' = results[1...,`=1+`x''..`=1+`x'']
putexcel F`=6+`i'+`x'' = matrix(m`x'), nformat("0.0%")	
}
}

** Age, Wealth **	
local i=-6
foreach var in age_cat2 wealth_quintile {
local i=`i'+6
foreach x of num 1/5 {
* All
tab `var' if `var' == `x' & analytic_pop_all == 1
putexcel C`=8+`i'+`x'' = `r(N)'
svy, subpop(analytic_pop_all): proportion `var'
matrix results = e(b)
matrix m`x' = results[1...,`x'..`x']
putexcel D`=8+`i'+`x'' = matrix(m`x'), nformat("0.0%")
* Diabetes
tab `var' if `var' == `x' & analytic_pop_diab == 1
putexcel E`=8+`i'+`x'' = `r(N)'
svy, subpop(analytic_pop_diab): proportion `var' 
matrix results = e(b)
matrix m`x' = results[1...,`x'..`x']
putexcel F`=8+`i'+`x'' = matrix(m`x'), nformat("0.0%")
}
}
...
* All
tab countryGDPclass if countryGDPclass == `x' & analytic_pop_all == 1
putexcel C`=25+`x'' = `r(N)'
svy, subpop(analytic_pop_all): proportion countryGDPclass
matrix results = e(b)
matrix m`x' = results[1...,`x'..`x']
putexcel D`=25+`x'' = matrix(m`x'), nformat("0.0%")
* Diabetes
tab countryGDPclass if countryGDPclass == `x' & analytic_pop_diab == 1
putexcel E`=25+`x'' = `r(N)'
svy, subpop(analytic_pop_diab): proportion countryGDPclass 
matrix results = e(b)
matrix m`x' = results[1...,`x'..`x']
putexcel F`=25+`x'' = matrix(m`x'), nformat("0.0%")	
}

** World region
foreach x of num 1/6 {
* All
tab worldregion6 if worldregion6 == `x' & analytic_pop_all == 1
putexcel C`=29+`x'' = `r(N)'
svy, subpop(analytic_pop_all): proportion worldregion6
matrix results = e(b)
matrix m`x' = results[1...,`x'..`x']
putexcel D`=29+`x'' = matrix(m`x'), nformat("0.0%")
* Diabetes
tab worldregion6 if worldregion6 == `x' & analytic_pop_diab == 1
putexcel E`=29+`x'' = `r(N)'
svy, subpop(analytic_pop_diab): proportion worldregion6 
matrix results = e(b)
matrix m`x' = results[1...,`x'..`x']
putexcel F`=29+`x'' = matrix(m`x'), nformat("0.0%")	
}


*** TABLE S6:  MEDICATION USE PATTERNS AMONG DIABETICS BY COUNTRY ***	(table corresponding to figure 2)															
	
use "$data/HPACC_diab_meds_simul_final.dta", clear
cd "$results"
 
putexcel set table2.xls, modify
...
*** Table S7: Insulin use LIC vs MIC (diagnosed vs all) ***			
...
svy, subpop(analytic_pop_diab) over(LIC): proportion insulin_new				
lincom _b[Yes:LIC] - _b[Yes:MIC]

svy, subpop(analytic_pop_diab) over(LIC): proportion oral_alldia_new,			
...
svy, subpop(analytic_pop_diab_diag) over(LIC): proportion insulin_new			
lincom _b[Yes:LIC] - _b[Yes:MIC]

svy, subpop(analytic_pop_diab_diag) over(LIC): proportion oral_alldia_new		
lincom _b[_prop_2:LIC] - _b[_prop_2:MIC]
...
***** Table S8: CONTROLLED DIABETES ******

use "$data/HPACC_diab_meds_simul_final.dta", clear
cd "$results"

svy, subpop(analytic_pop_diab_diag if dia_therapy == 1): proportion con_diagdia_7_NEW

svy, subpop(analytic_pop_diab_diag) over(treat_diagdia_2): proportion con_diagdia_7_NEW

mdesc con_diagdia_7_NEW if analytic_pop_diab_diag == 1 & dia_therapy == 1 		// 72 obs missing control status (did not fast, had no A1c measure, and FBG above 10.1) 

count if analytic_pop_diab_diag == 1 & treat_diagdia_2 == 1 & con_diagdia_7_NEW != .
count if analytic_pop_diab_diag == 1 & treat_diagdia_2 == 1 & con_diagdia_7_NEW == 1 
count if analytic_pop_diab_diag == 1 & treat_diagdia_2 == 1 & con_diagdia_7_NEW == 0 

count if analytic_pop_diab_diag == 1 & treat_diagdia_2 == 2 & con_diagdia_7_NEW != .
count if analytic_pop_diab_diag == 1 & treat_diagdia_2 == 2 & con_diagdia_7_NEW == 1 
count if analytic_pop_diab_diag == 1 & treat_diagdia_2 == 2 & con_diagdia_7_NEW == 0 

count if analytic_pop_diab_diag == 1 & treat_diagdia_2 == 3 & con_diagdia_7_NEW != .
count if analytic_pop_diab_diag == 1 & treat_diagdia_2 == 3 & con_diagdia_7_NEW == 1 
count if analytic_pop_diab_diag == 1 & treat_diagdia_2 == 3 & con_diagdia_7_NEW == 0 


***** Table S9: LIFESTYLE ADVICE ******

use "$data/HPACC_diab_meds_simul_final.dta", clear
cd "$results"

tab dia_adv		// "1 = has received advice to exercise, to lose weight, to reduce dietary fat, or a special diabetic diet"

gen dia_adv_main = 0
replace dia_adv_main = 1 if dia_adv_pa == 1 | dia_adv_diet == 1 | dia_adv_bw == 1 
replace dia_adv_main = . if mi(dia_adv_pa) & mi(dia_adv_diet) & mi(dia_adv_bw)

*bys country: mdesc dia_adv_main							// Missing for El Salvador, Marshall Islands, Mexico, and South Africa	
drop if country == "El Salvador" | country == "Marshall Islands" | country == "Mexico" | country == "South Africa"
count if analytic_pop_diab == 1								// 18,116	
count if analytic_pop_diab == 1	& !mi(dia_adv_main)			// N = 17,667 (2.5% missingness)		

gen trt_bin = .
replace trt_bin = 0 if treat_diagdia == 0
replace trt_bin = 1 if treat_diagdia == 1 |  treat_diagdia == 2 | treat_diagdia == 3

svy, subpop(analytic_pop_diab_diag) over(trt_bin): proportion dia_adv_main

count if analytic_pop_diab_diag == 1 & dia_adv_main == 1 & trt_bin == 0
count if analytic_pop_diab_diag == 1 & dia_adv_main == 0 & trt_bin == 0

count if analytic_pop_diab_diag == 1 & dia_adv_main == 1 & trt_bin == 1
count if analytic_pop_diab_diag == 1 & dia_adv_main == 0 & trt_bin == 1
