...
replace dia_med_new = . if dia_med_new > 1		// placeholder = .
replace insulin_new = . if insulin_new > 1		// placeholder = .
...
* Treatment status among all individuals with diabetes 					// For now, 641 diabetics missing only EITHER dia_med_new OR insulin_new (see below) effectively excluded 
...
replace treat_alldia_new = . if clin_dia_main == 1 & (insulin_new ==. | dia_med_new ==.)					// 1,292 changes (= diabetics missing EITHER meds OR insulin (or both))
...
* Treatment status among all individuals with DIAGNOSED diabetes 		// Currently ignores/excludes 396 diabetics missing diagnosis status (all of them miss insulin & meds data, so will get excluded because of that) (see below) 
gen  treat_diagdia = 0 if clin_dia_main == 1 & hbg_new == 1 			// For now, 529 DIAGNOSED diabetics missing only EITHER dia_med_new OR insulin_new (see below) effectively excluded
...
mdesc hbg_new if clin_dia_main == 1 																	// 616 --> currently "ignored" and thus treat_diagdia =.
replace treat_alldia = . if clin_dia_main == 1 & hbg_new == 1 & (insulin_new ==. | dia_med_new ==.)		// 0 changes 
count if  clin_dia_main == 1 & hbg_new == 1 & (insulin_new ==. | dia_med_new ==.)						// 510 (= DIAGNOSED diabetics missing EITHER meds OR insulin (or both))				
...
* Control status (among treated) (HbA1c of 8% or equivalent)							// Note: Untreated by definition cannot be controlled!

/*
> Criteria for controlled diabetes:
- Participants had A1c <7%
- Participants had FBG <8.6, irrespective of fasting status 					// Individuals who did not fast, yet are already below threshhold, would have had even lower values if they fasted

> Criteria for uncontrolled diabetes:
- Participants had A1c >=7% (even if FBG <8.6)
- Participants had FBG >=8.6 and fasted (or were assumed to fast); unless they had A1c <6%
- CAVE: For participants who did not fast AND have no A1c measure, we cannot ascertain if their FBG is truly above 8.6, or because this is due to them not fasting.

> Criteria for missingness: 
- Participant did not fast and have FBG is >=8.6 and have no A1c measure
- No biomarker
*/

gen con_diagdia_7_NEW = . 

replace con_diagdia_7_NEW = 1 if clin_dia_main == 1 & dia_therapy == 1 & (hba1c_p <7 & hba1c_p !=.) 
replace con_diagdia_7_NEW = 1 if clin_dia_main == 1 & dia_therapy == 1 & (fbg_new <8.6 & fbg_new !=.)

replace con_diagdia_7_NEW = 0 if clin_dia_main == 1 & dia_therapy == 1 & (hba1c_p >=7 & hba1c_p !=.) 
replace con_diagdia_7_NEW = 0 if clin_dia_main == 1 & dia_therapy == 1 & ((fbg_new >=8.6  & fbg_new !=. & fast_new !=0) & (hba1c_p == . | (hba1c_p !=. & hba1c_p >=7)))	

replace con_diagdia_7_NEW = . if fast_new == 0 & hba1c_p == . & (fbg_new >=8.6  & fbg_new !=.)
replace con_diagdia_7_NEW = . if biomarker == .

label values con_diagdia_7_NEW bin

mdesc con_diagdia_7_NEW if clin_dia_main == 1 & dia_therapy == 1 	// 94 individuals who did not fast, had no A1c measure, and FBG above 8.6 

* Alternative treatment variables (exclude "no medication" group)
gen treat_diagdia_2 = treat_diagdia
replace treat_diagdia_2 = . if treat_diagdia_2 == 0
label values treat_diagdia_2 trt 
...
global group2 Chile China Indonesia Mexico Romania South_Africa El_Salvador Comoros Costa_Rica Togo Seychelles
global group3 Cambodia Eritrea Fiji Iran Laos Lesotho Liberia Libya Palau Rwanda Samoa Tanzania Vanuatu Zanzibar 
...
label define qst 1 "Meds before insulin" 2 "Multiple choice" 3 "Meds after insulin" 
label values qst_type qst
...
replace country_sngl = "South Africa{sup:{bf}2}" if country_sngl == "South_Africa{sup:{bf}2}" 	
replace country_sngl = "El Salvador{sup:{bf}2}" if country_sngl == "El_Salvador{sup:{bf}2}" 

* Destring year variable & generate binary variable
gen year_num = year
replace year_num = "2009.5" if year == "2009-10"
replace year_num = "2012" if year == "2011-2013"
replace year_num = "2014.5" if year == "2014-2015"
replace year_num = "2015.5" if year == "2015-2016"
destring year_num, replace

gen year_grp = .
replace year_grp = 0 if year_num <2015
replace year_grp = 1 if year_num >=2015
