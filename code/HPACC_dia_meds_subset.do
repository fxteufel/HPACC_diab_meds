...
drop if age ==. & age_ll25 !=1 		// 1,528 individuals (0.38%) excluded	(Only if surveys included individuals <25 years)
...
count 								// 223,283
count if clin_dia_main == 1 		// 21,715				// Note: only 159 people with diabetes are pregnant

mdesc hbg_new if clin_dia_main == 1 		// 0
...
* Generate weight for analytic subpop(s) *		
...
** SET SURVEY DESIGN: Define before each analysis **	// "singleunit(centered)" implies that strata with a single sampling unit are centered at the grand mean instead of the stratum mean

svyset psu_num[pw = weq_w3_analytic_all], strata(stratum_num) singleunit(centered)  		
